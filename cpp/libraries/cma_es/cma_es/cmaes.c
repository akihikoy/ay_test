 ...
 * */
{
  ...
  if (!t->sp.flgsupplemented) {
    ...
    if (!isNoneStr(t->sp.filename)) /* TODO: should this be done in readpara_SupplementDefaults? */
      cmaes_readpara_WriteToFile(&t->sp, "/tmp/actparcmaes.par");
  }
            ...
            char const *s4)
{
  ...
  printf(" *** CMA-ES ABORTED, see /tmp/errcmaes.err *** \n");
  fflush(stdout);
  exit(1);
}
                          ...
                          char const *s3, char const *s4)
{
  ...
  */
  time_t t = time(NULL);
  FILE *fp = fopen( "/tmp/errcmaes.err", "a");
  if (!fp)
    {
      printf("\nFATAL ERROR: %s\n", s2 ? szCat(s1, s2, s3, s4) : s1);
      printf("cmaes_t could not open file '/tmp/errcmaes.err'.");
      printf("\n *** CMA-ES ABORTED *** ");
      fflush(stdout);
      exit(1);
    }
