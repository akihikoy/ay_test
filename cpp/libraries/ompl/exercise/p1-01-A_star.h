...
bool TAStarSearch::Solve()
{
  ...
  while(!openset.empty())
  {
    TState current= find_lowest(openset, f_score);
    if(current==goal_)
      return true;

    openset.remove(current);
    closedset.push_back(current);
    std::list<TState> neighbor_nodes= successors_function_(current);
    for(std::list<TState>::const_iterator neighbor(neighbor_nodes.begin()),n_last(neighbor_nodes.end());
        neighbor!=n_last; ++neighbor)
    {
      if(is_in(*neighbor,closedset))
        continue;
      TValue tentative_g_score= g_score[current] + cost_function_(current,*neighbor);

      if(!is_in(*neighbor,openset) || tentative_g_score<g_score[*neighbor])
      {
        inverse_policy_[*neighbor]= current;
        if(callback_on_updated_)  callback_on_updated_(*this, current, *neighbor);
        g_score[*neighbor]= tentative_g_score;
        f_score[*neighbor]= g_score[*neighbor] + heuristic_cost_function_(*neighbor, goal_);
        if(!is_in(*neighbor,openset))
