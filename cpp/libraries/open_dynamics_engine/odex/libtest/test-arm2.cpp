...
{
  dReal target[4]={0.5,0.8,0.8,0.0};
  ...
  target[2]= 0.5*(std::sin(0.2+0.2*env.Time())+1.0);
  target[3]= 0.5*(std::sin(0.2+0.5*env.Time()));
  ...
  for(int j(0);j<1;++j)
  {
    robot.SetAngVelUniversal1(j,kp*(target[3]-robot.GetAngleUniversal1(j)));
    robot.SetAngVelUniversal2(j,kp*(target[3]-robot.GetAngleUniversal2(j)));
  }
