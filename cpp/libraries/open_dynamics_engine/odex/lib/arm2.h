...
void TDynRobot::Create(dWorldID world, dSpaceID space)
{
  ...
  // arm1
  {
    ...
    dMatrix3 R;
    // dRSetIdentity (R);
    dRFromAxisAndAngle (R,0.0,1.0,0.0,0.1*M_PI);
  ...
  {
    body_[i].create (world);
    body_[i].setPosition (0.0,0.0,1.7);
    dMass m;
    dMassSetCylinder (&m,1,/*z*/3,/*radius*/0.2,/*length*/0.6);
    // m.adjust (mass);
    body_[i].setMass (&m);
    link_cy_[icy].create (space,/*radius*/0.2,/*length*/0.6);
    link_cy_[icy].setBody (body_[i]);
    ++i;++icy;
  }
  ...
  // test
  {
    joint_u_[ju].create (world);
    joint_u_[ju].attach (body_[5],body_[4]);
    joint_u_[ju].setAnchor (0.0,0.0,1.4);
    joint_u_[ju].setAxis1 (1.0,0.0,0.0);
    joint_u_[ju].setAxis2 (0.0,1.0,0.0);
    joint_u_[ju].setParam(dParamFMax1,fmax);
    joint_u_[ju].setParam(dParamFMax2,fmax);
    ++ju;
  }
}
