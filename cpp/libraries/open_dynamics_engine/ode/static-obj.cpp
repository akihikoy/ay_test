...
/*override*/void TRobot1::Create(dWorldID world, dSpaceID space)
{
  ...
  for (int i=0; i<9; i++)
  {
    ...
    joint_b_[i].setAnchor (0,0,k+0.4);
  }
}
...
/*override*/void TGeom1::Create(dWorldID world, dSpaceID space)
{
  ...
  {
    link_b_[i].create (space,0.2,0.04,0.2);
  ...
  for(int i(4);i<1000;++i)
  {
    link_b_[i].create (space,1.0,0.1,1.0);
    // link_b_[i].create (space,0.1,0.1,0.1);
    link_b_[i].setPosition (dRandReal()*20.0-10.0,dRandReal()*20.0-10.0,0);
    dRFromAxisAndAngle (R,dRandReal()*2.0-1.0,dRandReal()*2.0-1.0,
                            dRandReal()*2.0-1.0,dRandReal()*10.0-5.0);
    link_b_[i].setRotation (R);
  }*/
...
void TEnvironment::StepSim(const double &time_step)
{
  ...
  robot_.Body().front().addForce (0,0,1.0*(std::cos(angle)+1.0));
