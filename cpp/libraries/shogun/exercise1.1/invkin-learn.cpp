...
int main (int argc, char **argv)
{
  ...
  if(LDIM==2)
  {
    feat= LoadMatFromFile("out-2d-pos.dat",num,dim,max_sample);
    lab[0]= LoadVecFromFile("out-2d-angle.dat",tmp1,tmp2,0,max_sample);
    assert(tmp1==num);
    lab[1]= LoadVecFromFile("out-2d-angle.dat",tmp1,tmp2,1,max_sample);
    assert(tmp1==num);
  }
  else
  {
    feat= LoadMatFromFile("out-3d-pos.dat",num,dim,max_sample);
    lab[0]= LoadVecFromFile("out-3d-angle.dat",tmp1,tmp2,0,max_sample);
    assert(tmp1==num);
    lab[1]= LoadVecFromFile("out-3d-angle.dat",tmp1,tmp2,1,max_sample);
    assert(tmp1==num);
    lab[2]= LoadVecFromFile("out-3d-angle.dat",tmp1,tmp2,2,max_sample);
    assert(tmp1==num);
  }
