...
int main(int,char**)
{
  ...
  float64_t *feat,*lab[LDIM];
  int tmp1,tmp2;
  LoadDataFromFile("d/input2.dat",feat,num,dim);
  LoadDataFromFile("d/output2.dat",lab[0],tmp1,tmp2,0);
  assert(tmp1==num);
  LoadDataFromFile("d/output2.dat",lab[1],tmp1,tmp2,1);
  assert(tmp1==num);
  ...
  for(int i(0);i<LDIM;++i) out_labels[i]= svm[i]->classify(test_features);
