//-------------------------------------------------------------------------------------------
/*! \file    leastsq.cpp
    \brief   Least squares with liboctave
...
int main(int argc, char**argv)
{
  ...
  Srand();
  TMatrix Theta(N_sample, NX*NY, 0.0);
  TVector V(N_sample,0.0);
  {
  ...
  TReal comp_time_u(GetUserTime()), comp_time_a(GetCurrentTime());
  {
    if(solver==sLeastSquares)
    {
      ...
      theta= (Theta.transpose()*Theta+lambda*GetEye(GenSize(theta))).inverse()*Theta.transpose() * V;
    }
    else if(solver==sSVD)
    {
