...
* In the terminal command line simply type::
  ...
  **upgrade** an existing installation the ``-U`` flag is available in
  both cases. If you never heard of ``pip``, `see here`__.
...
Installation might require root privileges and therefore commands might
need to be prepended with ``sudo``.
