...
class TZTS(object):
  ...
  def ReadingLoop(self):
    ...
    '''
    try:
      self.ser.write('XFC\r')
      raw= self.ser.read_until('\r')
      if raw[:3]=='XFC':
        units= [ZTS_UNIT_CODE[raw[i:i+2]] for i in range(3,15,2)]
      else:  units= ['None']*6
      print 'Units:',units

      if self.cnt:  self.ser.write('XAG\r')  #NOTE: Stop by XAS. No need to use sleep.
      n= 0
      while self.running:
        if not self.cnt:  self.ser.write('XAR\r')  #Alternative to XAR.
        raw= self.ser.read_until('\r')
      ...
      if self.cnt:  self.ser.write('XAS\r')  #Use with XAG.
