...
if __name__=='__main__':
  ...
  #with open('/tmp/p.dat','w') as fp:
    #for x in np.arange(0.0,1.0,0.01):
      #fp.write('{0} {1} {2} {3} {4}\n'.format(x, F2MLin0(x,p), p[2]*x, ((p[2]-p[3])*p[0])+p[3]*x, Sigmoid(x,p[0],p[1])))
  #print 'Plot by:'
  ...
  ax.plot(X, F2MLin0(X,p), linewidth=3, label='F2MLin0')
  ax.plot(X, p[2]*X, linewidth=1, label='Lin-1')
  ax.plot(X, ((p[2]-p[3])*p[0])+p[3]*X, linewidth=1, label='Lin-2')
