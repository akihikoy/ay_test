#!/usr/bin/python3
#\file    grad_descent2.py
#\brief   Gradient descent algorithms (e.g. Ada-Delta; ver.2).
...
class TGradientDescent(TFirstOrderOptimizer):
...
class TAdaDelta(TFirstOrderOptimizer):
  ...
  def Step(self, param, grad, state):
    r,s= state
    if self.normalize_grad:  grad= grad/la.norm(grad)
    param,r,s= StepAdaDeltaV(param,grad,r,s, self.rho,self.eps)
    return param, (r,s)
