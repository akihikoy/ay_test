  ...
  def decom(idxes_poly, depth=0):
    ...
    for i_point in idxes_poly:
      ...
      for j_point in idxes_poly:
        ...
        tmp1= decom(idxes_tmp1, depth+1) if len(idxes_tmp1)>3 else []
        tmp2= decom(idxes_tmp2, depth+1) if len(idxes_tmp2)>3 else []
        tmp1= tmp1+tmp2
        if ndiags is None or len(tmp1)<ndiags:
          min_diags= tmp1+[[i_point,j_point]]
          ndiags= len(tmp1)
          print('{}-th min_diags= {} (ndiags={})'.format(depth,min_diags,ndiags))
