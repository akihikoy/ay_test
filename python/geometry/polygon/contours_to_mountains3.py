...
def MakeValleyMap(valleys, width, height):
  ...
  for subcontour in reversed(valleys):
    while True:
      if id(subcontour) not in list(map(id,valleys_and_outers)):
  ...
  for subcontour in reversed(valleys):
    for subcontour_inner in subcontour.inner:
      if id(subcontour_inner) not in list(map(id,mountains)):
...
if __name__=='__main__':
  ...
  cv_contours= pickle.load(open('data/mlcontours4a.dat','rb'), encoding='latin1')
