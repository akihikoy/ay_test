...
def GaussianM(x, x0, var, a=1.0):
  diff= np.abs(x - x0).max()  #This is MAX NORM
  dot_product= diff * diff
  return a * np.exp(dot_product / (-2.0 * var))
...
class TLWR:
  def __init__(self):
    raise Exception('This TLWR is no longer maintained. Use lwr_incr2.TLWR with "maxg" kernel.')
  ...
  def Update(self,x,y):
    ...
    if len(self.Closests)==1:
      ...
      self.Closests[0]= 1
      self.CDists= [DistM(self.DataX[0],self.DataX[1])]*2
  ...
  def AutoWidth(self, c_min=0.01, c_max=1.0e6, c_gain=0.7):
    ...
    C= [0.0]*N
    for n in range(N):
      C[n]= max( c_min, min([c_gain*DistM(self.DataX[n],self.DataX[d]) if d!=n else c_max for d in range(N)]) )
    #print C
    return C
