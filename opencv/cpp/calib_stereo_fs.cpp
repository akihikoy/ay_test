// From OpenCV sample.
// g++ -g -Wall -O2 -o calib_stereo_fs.out calib_stereo_fs.cpp -I$HOME/.local/include -L$HOME/.local/lib -Wl,-rpath=$HOME/.local/lib -lopencv_core -lopencv_calib3d -lopencv_features2d -lopencv_imgproc -lopencv_imgcodecs -lopencv_highgui -I/usr/include/opencv4
...
static void
StereoCalib(const vector<string>& imagelist, Size boardSize, float squareSize, bool useCalibrated=true, bool showRectified=true)
{
        ...
        for( k = 0; k < 2; k++ )
        {
            ...
            cornerSubPix(img, corners, Size(11,11), Size(-1,-1),
                         TermCriteria(cv::TermCriteria::MAX_ITER+cv::TermCriteria::EPS,
                                      30, 0.01));
            vector<Point2d>& cornersd = imagePoints[k][j];
            for(vector<Point2f>::const_iterator citr(corners.begin()),cend(corners.end()); citr!=cend; ++citr)
              cornersd.push_back(Point2d(citr->x,citr->y));
        }
        if( k == 2 )
        {
    ...
    for( i = 0; i < nimages; i++ )
    {
        for( j = 0; j < boardSize.height; j++ )
            for( k = 0; k < boardSize.width; k++ )
                objectPoints[i].push_back(Point3d(k*squareSize, j*squareSize, 0));
    }
    ...
    // Mat objPt, imgPt, imgPt2, npoints2;
    // collectCalibrationData(objectPoints, imagePoints[0], imagePoints[1],
                            // objPt, imgPt, &imgPt2, npoints2);
    // double rms = fisheye::stereoCalibrate(objPt, imgPt, imgPt2,
    ...
    // Mat F= findFundamentalMat(imagePoints[0], imagePoints[1], FM_8POINT, 0, 0);
...
// epipolar geometry constraint: m2^t*F*m1=0
    ...
    */
    ...
    fisheye::stereoRectify(cameraMatrix[0], distCoeffs[0],
                  ...
                  CALIB_ZERO_DISPARITY, imageSize);
