...
// }
    ...
    morphed.setTo(cv::Scalar(100,100,100));
    ...
    t_src.clear();
    t_dst.clear();
    t_src.push_back(cv::Point2f(100,100));
    t_src.push_back(cv::Point2f(540,240));
    t_src.push_back(cv::Point2f(100,380));
    t_dst.push_back(cv::Point2f(100,100));
    t_dst.push_back(point_morph);
    t_dst.push_back(cv::Point2f(100,380));
...
// void MorphTriangle(cv::Mat &src, cv::Mat &dst, std::vector<cv::Point2f> &t_src, std::vector<cv::Point2f> &t_dst)
    {
      ...
      cv::multiply(morphed(r_dst), cv::Scalar(1,1,1) - mask, morphed(r_dst));
      morphed(r_dst) = morphed(r_dst) + dst_r;
    }
