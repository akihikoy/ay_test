//-------------------------------------------------------------------------------------------
/*! \file    simple_blob_tracker5.cpp
    \brief   Blob tracker test code ver.5.
    \author  Akihiko Yamaguchi, info@akihikoy.net
    \version 5.1
    \date    Jul.15, 2021

g++ -g -Wall -O2 -o simple_blob_tracker5.out simple_blob_tracker5.cpp -lopencv_core -lopencv_imgproc -lopencv_features2d -lopencv_highgui -lopencv_videoio -I/usr/include/opencv4

Run:
  $ ./simple_blob_tracker5.out
  OR
  $ ./simple_blob_tracker5.out CAMERA_NUMBER
...
TBlobTracker2Params::TBlobTracker2Params()
{
  ...
  NonZeroMin= 0.0; // Minimum ratio of nonzero pixels in ROI over original keypoint area.
  NonZeroMax= 7.0; // Maximum ratio of nonzero pixels in ROI over original keypoint area.
  VPMax= 100.0;  // Maximum position change (too large one might be noise)
  VSMax= 20.0;  // Maximum size change (too large one might be noise)
  NReset= 7;  // When number of tracking failure in a row exceeds this, tracking is reset
  // For calibration:
  DistMaxCalib= 2.0;
  DSMaxCalib= 2.0;
...
void TBlobTracker2::Preprocess(const cv::Mat &img, cv::Mat &img_th)
{
  if(params_.ThresholdingImg)
  {
    ...
    cv::inRange(img_th,
                ...
                cv::Scalar(params_.ThreshH2, params_.ThreshS2, params_.ThreshV2), img_th);
  ...
  for(int f(0);;++f)
  {
    ...
    else if(c=='C')
    {
      ...
      if(trackbar_mode==1)
      {
        ...
        CreateTrackbar<int>("ThreshH1", win, &tracker.Params().ThreshH1, 0, 255, 1, &TrackbarPrintOnTrack);
        CreateTrackbar<int>("ThreshS1", win, &tracker.Params().ThreshS1, 0, 255, 1, &TrackbarPrintOnTrack);
        CreateTrackbar<int>("ThreshV1", win, &tracker.Params().ThreshV1, 0, 255, 1, &TrackbarPrintOnTrack);
        CreateTrackbar<int>("ThreshH2", win, &tracker.Params().ThreshH2, 0, 255, 1, &TrackbarPrintOnTrack);
        CreateTrackbar<int>("ThreshS2", win, &tracker.Params().ThreshS2, 0, 255, 1, &TrackbarPrintOnTrack);
        CreateTrackbar<int>("ThreshV2", win, &tracker.Params().ThreshV2, 0, 255, 1, &TrackbarPrintOnTrack);
      ...
      else if(trackbar_mode==3)
      {
        ...
        CreateTrackbar<int>("NCalibPoints:", win, &tracker.Params().NCalibPoints, 0, 60, 1, &TrackbarPrintOnTrack);
        CreateTrackbar<float>("DSEmp:", win, &tracker.Params().DSEmp, 0.0, 50.0, 0.1, &TrackbarPrintOnTrack);
