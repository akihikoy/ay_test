...
g++ -g -Wall -O2 -o cv2-video_fps.out cv2-video_fps.cpp -lopencv_core -lopencv_highgui -lopencv_videoio -I/usr/include/opencv4
*/
//-------------------------------------------------------------------------------------------
...
int main(int argc, char**argv)
{
  ...
  vin.set(cv::CAP_PROP_FPS, 15);  // TEST:Is setting FPS affects get(FPS)?
                                 // Result: No, set(FPS) does not affects neither get(FPS) or actual vin reading.

  std::cerr<<"FPS from vin.get(CAP_PROP_FPS): "<<vin.get(cv::CAP_PROP_FPS)<<std::endl;
