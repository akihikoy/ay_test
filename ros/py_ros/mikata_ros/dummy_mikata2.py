...
class TRobotDummyMikata(object):
  def __init__(self):
    ...
    self.js.position= [0.0]*len(self.js.name)
    self.js.velocity= [0.0]*len(self.js.name)
    self.js.effort= [0.0]*len(self.js.name)
  ...
  def FollowTraj(self, traj):
    ...
    splines= [TCubicHermiteSpline() for d in range(len(self.js.name))]
    ...
    while all(((rospy.Time.now()-t0)<t_traj[-1], self.follow_traj_active, not rospy.is_shutdown())):
      ...
      q= [splines[d].Evaluate(t) for d in range(len(self.js.name))]
      #print t, q
      with self.js_locker:
...
if __name__=='__main__':
  rospy.init_node('dummy_mikata')
  robot= TRobotDummyMikata()
  rospy.spin()
